...
const mapPrefixItems = (items, state) => {
    if (items.length) {
        return items.map((item, index) => (0, schema_1.createSchemaObject)(item, state, [`tuple item ${index}`]));
    }
    return undefined;
};
const mapItemProperties = (items, rest, state) => {
    ...
    if ((0, openapi_1.satisfiesVersion)(state.components.openapi, '3.1.0')) {
        ...
        return {
            items: (0, schema_1.createSchemaObject)(rest, state, ['tuple items']),
            ...(prefixItems && { prefixItems }),
        };
    }
    if (!rest) {
    ...
    return {
        ...(prefixItems && {
            items: {
                oneOf: [
                    ...prefixItems,
                    (0, schema_1.createSchemaObject)(rest, state, ['tuple items']),
                ],
            },
        }),
    };
